...
instance Available TimUpdate (Maybe User) where
    getIt (TimUpdate x) = (Just . User <$> (getField "markup" x >>= getField "user_id"))
                          <|> return Nothing
instance Available TimRender (Maybe User) where
    getIt (TimRender x) = (Just . User <$> (getField "markup" x >>= getField "user_id"))
                          <|> return Nothing
